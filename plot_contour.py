...
ncfile = f'../{run}/'+sys.argv[1]+ '_0001.nc'
fname = sys.argv[1]
for place in stn_node:
    index = stn_node[place]
    fvcom = FileReader(ncfile, variables=['zeta', 'salinity','temp','km','kh'], dims={'time': slice(0,8700), 'node': index,'siglev':slice(1,31)})
...
#dynamic
    time.plot_surface(z, fvcom.data.salinity, fill_seabed=True, h_offset=0, h_min=fvcom.grid.h,#norm=Normalize(vmin=fvcom.data.salinity.min(), vmax=fvcom.data.salinity.max()))
    ...
    """
    #fvcom = FileReader(ncfile, variables=['zeta', 'kh'], dims={'time': slice(0,8700), 'node': index,'siglev':slice(1,31)})
    time = Time(fvcom, figsize=(35,12), cmap = 'jet',cb_label='{} ({})'.format(fvcom.atts.kh.long_name,fvcom.atts.kh.units))
    z = unstructured_grid_depths(fvcom.grid.h, fvcom.data.zeta, fvcom.grid.siglay)
    time.plot_surface(z, fvcom.data.kh, fill_seabed=True, h_offset=0, h_min=fvcom.grid.h,norm=LogNorm(vmin=10**-4, vmax=10**-2))
    time.axes.set_ylabel('Depth (m)')
    time.figure.savefig('./png/contour/kh'+run+fname+'_'+place+'.png')
    print('savefig,path=./png/contour/kh'+run+fname+'_'+place+'.png')

    time = Time(fvcom, figsize=(35,12), cmap = 'jet',cb_label='{} ({})'.format(fvcom.atts.km.long_name,fvcom.atts.km.units))
    z = unstructured_grid_depths(fvcom.grid.h, fvcom.data.zeta, fvcom.grid.siglay)
    time.plot_surface(z, fvcom.data.km, fill_seabed=True, h_offset=0, h_min=fvcom.grid.h,norm=LogNorm(vmin=10**-3, vmax=10**-1))
    time.axes.set_ylabel('Depth (m)')
    time.figure.savefig('./png/contour/km'+run+fname+'_'+place+'.png')
    print('savefig,path=./png/contour/km'+run+fname+'_'+place+'.png')
