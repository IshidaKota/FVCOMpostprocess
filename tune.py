...
def plotter2(var_name,place,index):
    ...
    if sigmoid:
        ...
        f = './data/interp/'+place+'_'+var_name+'TPdepth_2020.csv'
    df = pd.read_csv(f)
    #make time series

    #日付軸の作成をしている(from simulation data)
    ...
    for i, k in enumerate(sigma):
       ...
       # print(type(column[0]),type(dates[0]),type(kokyo_df['date'][0]))
        ...
        if variable == 'salinity':
            if layer[i] == 'upper':
                axs[i].plot(kokyo_df['date2'][:18:2],kokyo_df['salinity'][:18:2],'go',label='kokyo')
            elif layer[i] == 'bottom':
                axs[i].plot(kokyo_df['date2'][1:18:2],kokyo_df['salinity'][1:18:2],'go',label='kokyo')
  
        axs[i].plot(dates,var[:,k, index],linewidth=1,label='simulation') 
        axs[i].plot(column,res[1:],linewidth=1,alpha=0.5,label='observation')
...
#run = sys.argv[1]
obcfile = f"../run27/input/input_testcase/TokyoBay_obc.dat" #specify where the obcfile is
...
#f = '../'+run+'/'+filename+ '_0001.nc'
ncfile_path = 'specify_your_path_to_ncfile'
fvcom = mod_fvcom.Fvcom2D(ncfile_path=ncfile_path, obcfile_path=obcfile, m_to_km=True, offset=False)
